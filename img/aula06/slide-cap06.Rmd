...
subtitle: "6 DESDOBRAMENTO DE GRAUS DE LIBERDADE DE TRATAMENTOS"
...
tb <- tb |> 
  mutate(
    across(
      where(is.character), ~str_replace(.,"\\.",",")
    )
  )
...
$$SQ_{Total} = (25,60^2+25,48^2+...+22,27^2) - \frac{531,84^2}{5 \times 5}$$
...
$$SQ_{Trat} = \frac{115,74^2+117,54^2+...+91,93^2}{5} - \frac{531,84^2}{5 \times 5}$$
...
$$\hat{Y_1}=2\cdot(115,74)+2\cdot(117,54)+2\cdot(120,75)-3\cdot(85,88)-3\cdot(91,95)$$

$$\hat{Y_1}=174,63\;cm$$

$$\sum_{i=1}^Ic_i^2=2^2+2^2+2^2+(-3)^2+(-3)^2=30 \text{ e }r=5$$
...
$$SQY_2=\frac{\hat{Y_2^2}}{r\sum_{i=1}^Ic_i^2}=\frac{(-6,81)^2}{5\times 6}=1,5459$$
...
Valores de F da tabela para (1 x 20 GL):
$$\begin{cases} 5\%=4,35 \\ 1\%=8,10 \end{cases}$$
---
**Verificação**: 
$$\frac{17,11+0,13+0,09+0,31}{4}=4,41=F_{Trat}$$
