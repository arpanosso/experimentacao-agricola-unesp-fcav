...
Por exemplo, num experimento no delineamento inteiramente casualizado com $5$ tratamentos e $5$ repetições, a casualização dos tratamentos seria feita sorteando-se para cada uma das $20$ parcelas do experimento uma combinação de tratamento e repetição, ou seja:
...
$$
\sum_{i=1}^I\sum_{j=i}^j\epsilon_{ij}^2=\sum_{i=1}^I\sum_{j=i}^j( y_{ij} - \mu - \tau_i)^2
$$
...
$$
\begin{cases} \sum_{i=1}^I\sum_{j=i}^Jy_{ij} - IJ\hat{m} -J\sum_{i=1}^I \hat{\tau_i} =0 \\ \sum_{j=1}^Jy_{ij}-J\hat{m}-J\hat{\tau_i}=0 \end{cases}
$$
