...
x: média amostral $(\bar{y})$ ; variância amostral $(s^2)$, etc...portanto é também uma variável aleatória.
...
OBS: a notação índice + implica na soma dos índices que ela substituí.
...
$$SQ_\text{Total} = \sum_{i=1}^a\sum_{j=1}^r \left( \frac{Y_{i+}}{r} - \hat{\mu} \right)^2$$
Manipulando algebricamente, temos:
...
$\frac{SQ_\text{Total}}{\sigma^2} \sim\chi^2_{(N-1)}$ sob $H_0$ verdadeira.

Temos que:

$\frac{SQ_\text{Tratamentos}}{\sigma^2} \sim\chi^2_{(a-1)}$ sob $H_0$ verdadeira.

e

$\frac{SQ_\text{Resíduo}}{\sigma^2} \sim\chi^2_{[a(r-1)]}$ sob $H_0$ verdadeira.
