...
subtitle: "4. TESTES PARA COMPARAÇÃO DE MÉDIAS"
...
### Aplicação da ANOVA (Análise de Variância)

Como exemplo de aplicação, utilizaremos os dados do trabalho: 

"Competição de adubos nitrogenados no abacaxizeiro", realizado por BRASIL SOBRINHO et al. (*Boletim Técnico Científico*, ESALQ, n. 12, 1962).

O ensaio foi instalado no **delineamento em blocos casualizados** com $6$ tratamentos e $4$ repetições, sendo que os blocos controlaram diferenças de fatores ambientais.

Cada parcela constituiu-se de $20$ plantas úteis no espaçamento de $1,5\;m$ entre linhas e $0,4\;m$ entre plantas. 

Sabe-se que:

$$SQ_\text{Total} = 185,20$$

$$SQ_\text{Tratamentos} = 168,29$$
...
## Quais foram os melhores tratamentos ?
...
Assim, se em um experimento temos $3$ tratamentos, cujas médias verdadeiras são $m_1,m_2$ e $m_3$ as relações:
...
$$\hat{Y_1}=\hat{m_1}-\hat{m_2} \\
\hat{Y_1}= 280-250=30$$
e
$$\hat{Y_2}=\hat{m_1}+\hat{m_2}-\hat{m_3}-\hat{m_4} \\ 
\hat{Y_2}= 280+250-320-275 = -65$$
...
Salientamos que a estimativa da variância é dada por:
...
Um contraste não carrega a informação a respeito de outro contraste.
...
| $\sum_{i=1}^I a_ib_i$ | $\rightarrow$ |**1** |**-1** |**0**  |

Assim, $+1 -1 + 0 = 0$, portanto, $Y_1$ e $Y_2$ são ortogonais entre si.
...
Na análise de variância, utilizamos o $QM_{Resíduo}$ como estimativa da variância $s^2$, então teremos, finalmente:

$$\hat{V}(\hat{Y}) = \left( {c^2_1} + {c_2^2} + \cdots + {c^2_I} \right) \frac{QM_\text{Resíduo}}{r}$$
...
$$\hat{Y_1} = 2\hat{m_3} - \hat{m_5} - \hat{m_6} = \\ \hat{Y_1} = 2.(24,58) - 28,85 - 28,30 = \\ \hat{Y_1} = -7,99\;kg$$
---
...
**5. Cálculo da estatística t de Student**
...
```{r,message=FALSE,error=FALSE,warning=FALSE,echo=FALSE,fig.width=15,fig.height=12}
...
data.frame(x=0,y=1) %>% 
  ...
  annotate('text', label = expression(paste("2,5%")), x = 2.7, y = .08,size=10,col="orange")+
  annotate('text', label = expression(paste("2,5%")), x = -2.7, y = .08,size=10,col="orange")+
  annotate('text', label = expression(paste("0,5%")), x = 6.7, y = .08,size=10,col="red")+
  annotate('text', label = expression(paste("0,5%")), x = -6.7, y = .08,size=10,col="red")+
  labs(y="Densidade",x="t")+
  annotate('text', label = "Região crítica", x = 6.6, y = 0.15,size=10,col="red")+
  annotate('text', label = "Região crítica", x = -6, y = 0.15,size=10,col="red")+
  geom_vline(xintercept = tobs, col= "blue",lwd=2) + 
  annotate('text', label = expression(paste(t[obs])), x = tobs+.8, y = .02,size=10,col="blue")+
  theme_minimal()
```
...
$$
t_{obs} = \frac{\hat{Y_2}}{s(\hat{Y_2})} = \frac{0,55}{0,57} = 0,96
$$
...
---
"Competição de adubos nitrogenados no abacaxizeiro", realizado por BRASIL SOBRINHO et al. (*Boletim Técnico Científico*, ESALQ, n. 12, 1962). O ensaio foi instalado no **delineamento em blocos casualizados** com $6$ tratamentos e $4$ repetições, sendo que os blocos controlaram diferenças de fatores ambientais. Cada parcela constituiu-se de $20$ plantas úteis no espaçamento de $1,5\;m$ entre linhas e $0,4\;m$ entre plantas. 

Dado a análise de variância:
...
$$C_I^2 = \begin{pmatrix} I \\ 2\end{pmatrix} = \frac{I!}{2!.(I-2)!} = \frac{I(I-1)}{2}\text{, contrastes}$$

É considerado um teste versátil, porém **não nos permite comparar 2 grupos de médias**.

---

Baseia-se na **diferença mínima significativa** (DMS oud d.m.s.) representado por $\Delta$ e dada por:

$$\Delta =q \cdot s(\hat{m})  \\ \Delta = q\frac{s}{\sqrt{r}}$$

onde,

$s$ é o desvio padrão residual, dado por $s = \sqrt{QM_\text{Resíduo}}$.

$r$ é o número de repetições com que as médias dos tratamentos foram calculadas.

$q$ é o valor da amplitude total estudentizada, valor encontrado em tabelas, em função do **Número de Tratamentos** do experimento $(n)$ e do **Número de graus de liberdade do resíduo** $(n')$, geralmente ao nível de $5\%$ de probabilidade. Portanto:

$$
\Delta = q \cdot \sqrt{\frac{QM_\text{Resíduo}}{r}}
$$
...
**2. Calcular a DMS, diferença mínima significativa ( $\Delta$ ):**
...
trat<-c("Test.","Sulf. amônio","Sal. Chile",
        "Ureia","Nitc.","Nitc.+S")
```
...
tukey<-HSD.test(medias, trat, GLres,QMres/r, 
                alpha=0.05, console = TRUE) 
```

---
```{r, fig.height= 7,fig.align='center',fig.width=12,warning=FALSE}
...
$$D= z_\alpha \cdot \frac{s}{\sqrt{r}} = z_\alpha  s(\hat{m})$$
...
$$D_k= z_\alpha \cdot \sqrt{\frac{QM_\text{Resíduo}}{r}}$$
---
...
**3- Calcular o valor de $D_k$ paa o número de médias correspondente, dado por.**

$$D_{k}= z_{(k,\alpha)} \cdot \frac{s}{\sqrt{r}}$$
...
$$\hat{Y_4} = \hat{m_5} - \hat{m_2} = 1,09^{ns}\;kg \\
...
duncan<-duncan.test(medias, trat, GLres, QMres/r, alpha=0.05,
                    console = TRUE)
```
...
# Conastrução do gráfico de colunas
bar.group(duncan$groups,ylim=c(0,max(medias))*1.2,cex.lab=2,
          las=1,xlab="Tratamentos",ylab="Produção (kg)",
...
Para exemplificar a aplicação do teste de Scheffé, vamos utilizar os dados do exemplo **4.2.1** de "Competição de adubos nitrogenados no abacaxizeiro". Neste exemplo os tratamentos, com suas respectivas médias de produção de abacaxi em kg/parcela, foram:
...
$$
Y = 2 \cdot m_2 + 2 \cdot m_3 + 2 \cdot m_4 - 3 \cdot m_5 - 3 \cdot m_6
$$

e sua estimativa é:
$$\hat{Y} = 2\hat{m_2} + .\hat{m_3} + 2\hat{m_4} - 3\hat{m_5} - 3\hat{m_6} \\
...
$$
\hat{V}(\hat{Y}) = \left[ 2^2+2^2+2^2+(-3)^2+(-3)^2\right] \cdot \frac{0,64}{4} = 4,80.
$$ 
...
$$
Y = 5 \cdot m_1 - m_2 - m_3 - m_4 - m_5 - m_6
$$
...
$$\hat{V}(\hat{Y}) = [5^2+(-1)^2+(-1)^2+(-1)^2+(-1)^2+(-1)^2].\frac{0,64}{4} = \\\hat{V}(\hat{Y}) = 30.\frac{0,64}{4} = 4,80$$
...
scheffe<-scheffe.test(medias, trat, GLres, QMres/r, alpha=0.05, 
                      console = TRUE)
```
